...
class AudioTranscriber:
    ...
    def transcribe_audio(self, audio_path: str) -> str:
        ...
        with sr.AudioFile(audio_path) as source:
            ...
            # Calculate chunks (process in 30-second segments)
            ...
            with tqdm(total=100, desc="Transcription progress", unit="%") as pbar:
                for i in range(num_chunks):
                    ...
                    # Read audio chunk
                    audio_data = self.recognizer.record(source, duration=duration, offset=offset)
                    
                    try:
                        # Transcribe chunk using Google Speech Recognition
                        text = self.recognizer.recognize_google(audio_data)
                        transcription_parts.append(text)
                    except sr.UnknownValueError:
                        transcription_parts.append("[Inaudible]")
                    except sr.RequestError as e:
                        print(f"\n⚠️  Error with speech recognition service: {e}")
                        transcription_parts.append("[Error]")
...
4. Add proper paragraph breaks for better readability
5. Maintain the original meaning and tone
6. Format as clean, professional text
...
Examples:
  ...
  audio-transcribe input.mp3 --prompt "Summarize the key points"
    ...
    if not api_key:
        ...
        print("Please provide it via --api-key or set GEMINI_API_KEY environment variable")
        sys.exit(1)
